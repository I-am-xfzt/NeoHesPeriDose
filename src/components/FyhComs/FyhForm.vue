          ...
          <component :is="'el-form-item'" v-bind="v.formItem">
            ...
            <template v-else>
              <component
                  ...
                  v-on:[v.methods.event]="(val) => handleEmit(val, v)"
              >
                <component
...
const handleEmit = (val, row: EmptyObjectType) => {

}
const getDictData = async () => {
  if (props.needLoadTableDict) {
    const {dicts} = useDict()
    await getDicts().then((res) => {
      dicts.value = res
      for (const key in res) {
        dictMaps[key] = res[key]
      }
    })
  }
}
