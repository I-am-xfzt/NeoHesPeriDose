...
import { MessageBox } from 'element-plus' // 以antd为例，可按需替换提示组件
import qs from 'qs'
...
abstract class BaseHttpClient {
  ...
  private async handleError(error: AxiosError<IErrorResponse>): Promise<any> {
    ...
    if (response?.data?.message || error.message) {
      MessageBox.error(`请求失败: ${errorMessage}`)
    } else {
      for (const code in this.errorCodeMap) {
        if (response?.status === Number(code)) {
          MessageBox.error(this.errorCodeMap[code])
          break
        }
      }
    }
  ...
  protected getToken(): string | null {
    return '' // 默认返回null，子类需要实现获取token的逻辑
  }
  ...
  protected abstract clearToken(): void {
    // 子类需要实现清除token的逻辑
  }
...
class UserApi extends BaseHttpClient {
  private readonly moduleUrl: string = '/user'
  constructor() {
    super(this.moduleUrl)
  }
  ...
  public login(username: string, password: string) {
    return this.post<{ userInfo: any }>('/login', { username, password })
  }
...
class OrderApi extends BaseHttpClient {
  ...
  static getOrderList(params: { page: number; size: number }) {
