...
/**
 * @description 递归处理菜单数组，为每个子菜单项设置 meta.activeMenu 为父级的 path
 * @param {Menu.MenuOptions[]} list 菜单列表
 * @return {Menu.MenuOptions[]}
 *
 */
export const setActiveMenuForChildren = (list: Menu.MenuOptions[], path?: string): Menu.MenuOptions[] => {
  return list.map((item: Menu.MenuOptions) => {
    // 创建新对象而不是直接修改原对象
    const newItem: Menu.MenuOptions = deepClone(item);
    path && (newItem.meta.activeMenu = path);
    if (newItem.children?.length) {
      // 递归处理子节点
      newItem.children = newItem.children.map(child => {
        const newChild: Menu.MenuOptions = deepClone(child);
        newChild.meta.activeMenu = path ?? newItem.path;
        // 如果还有子节点，继续递归处理
        if (newChild.children?.length) {
          newChild.children = setActiveMenuForChildren(newChild.children, newItem.path);
        }
        return newChild;
      });
    }
