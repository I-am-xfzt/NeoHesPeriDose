...
// 日期格式化
export function parseDate(time: string | number | Date, pattern: string) {
	if (arguments.length === 0 || !time) {
		return null;
	}
...
/**
 * 计算两个时间字符串的时间差
 * @param {string} time1 时间字符串1（格式：YYYY/MM/DD HH:mm:ss）
 * @param {string} time2 时间字符串2（格式：YYYY/MM/DD HH:mm:ss）
 * @return {object} 包含时差各单位的对象
 */
export function getTimeDifference(time1: string, time2: string) {
	...
	return {
		...
		days: diffDays,
		// 格式化后的时间差（天:时:分:秒）
		formatted: `${diffDays}天${remainingHours}时${remainingMinutes}分${remainingSeconds}秒`
	};
}
/**
 * 任意格式时间转换
 ...
 */
export const formatDateTime = (time: string | number | Date | null | undefined, fmt: string) => {
	if (time === 'Invalid Date' || time === null || time === undefined) return '' // 无效时间直接返回
	...
	for (const k in o) {
		if (new RegExp('(' + k + ')').test(fmt)) {
			...
			fmt = fmt.replace(RegExp.$1, (RegExp.$1.length === 1) ? String(value) : (('00' + value).substr(String(value).length)))
		}
	}
	return fmt
}
