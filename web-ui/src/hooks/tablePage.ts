import { type CellStyle, ElMessage } from "element-plus";
...
export interface TableStyle {
  ...
  maxHeight: (h: string | number) => string | number;
  rowStyle: (data: { row: any; rowIndex: number }) => CSSProperties;
  cellFunStyle: (data: { row: any; column: any; rowIndex: number; columnIndex: number }) => CSSProperties;
  [k: string]: any;
}
...
export function useTable(options?: BasicTableProps) {
   ...
   */
  const getDataList = (refresh: boolean = false) => {
   ...
   * 重置表单数据
   */
  const resetData = (refresh: boolean, call = () => {}) => {
    for (const k in state.queryForm) {
      state.queryForm[k] = undefined;
    }
    call();
    getDataList(refresh);
  };
