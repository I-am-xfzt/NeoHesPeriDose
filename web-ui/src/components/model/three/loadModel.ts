...
const createModelObject = <K extends GlbModelFilesType, V = GLTF>(
  key: K,
  value: V
): Record<K, V> => ({ [key]: value } as Record<K, V>)
...
export class ThreeModel {
  ...
  initLights(name: string) {
    ...
    if (!unSetOther) {
      ...
      const target = this.glbInfos?.find((v) =>
  ...
  async initGround() {
    ...
    const res = await Promise.all(
      loadArray.map((v) => textureLoader.load(v.path))
    )
  ...
  async init(
    ...
    this.initRender()
    this.initCamera()
    unLoad!?.sky && this.initAboutScene()
    this.initControls()
    const res = await Promise.all(
      getLoadModelOptions.map((v) => this.loadModel(...v))
    )
    this.glbInfos = res
    this.initLights(getLoadModelOptions[0][0])
    // this.setCameraView()
