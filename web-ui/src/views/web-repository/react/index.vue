<template>
  <div class="interview-questions page-container">
    <div class="interview-questions-content">
      ...
      <QuestionHeader />
      <!-- 搜索和筛选 -->
      <QuestionSearch @search="filteredQuestions" @addQuestion="addQuestion" :categories="[insert, ...react_repository_tag]" />
      <!-- 题目列表 -->
    ...
    <theDialog v-if="visible" v-model:visible="visible" title="添加React题目" @confirm="confirmHandleQuestion" @cancel="resetForm">
...
<script setup lang="ts" name="scss-respository">
...
const {
  ...
  currentChangeHandle,
  insert
} = useHandleQuestion("react");
const { react_repository_tag } = useDict("react_repository_tag");
</script>
