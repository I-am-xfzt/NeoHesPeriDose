<template>
  <div class="interview-questions page-container">
    <div class="interview-questions-content">
      ...
      <QuestionHeader />
      <!-- 搜索和筛选 -->
      <QuestionSearch @search="filteredQuestions" @addQuestion="addQuestion" :categories="[insert, ...scss_repository_tag]" />
      <!-- 题目列表 -->
    ...
    <theDialog v-if="visible" v-model:visible="visible" title="添加Scss题目" @confirm="confirmHandleQuestion" @cancel="resetForm">
...
const {
  ...
  currentChangeHandle,
  insert
} = useHandleQuestion("scss");
const { scss_repository_tag } = useDict("scss_repository_tag");
</script>
